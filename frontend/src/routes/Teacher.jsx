...
function Teacher() {
  ...
  useEffect(() => {
    console.log("Teacher dashboard cookies:", cookies);
  ...
  }, [cookies.roomId, cookies.teacherId, navigate]);
  ...
  useEffect(() => {
    const fetchExistingQuestions = async () => {
      ...
      try {
        ...
        const res = await api.get("http://localhost:3001/api/teachers/quizzes",{withCredentials:true}); 
  ...
  const handlePublishQuiz = async () => {
    ...
    try {
      await api.post("http://localhost:3001/api/teachers/quizzes", ques,{withCredentials:true});
  ...
  const handleViewStudent = async (studentId) => {
    try {
      const res = await api.get(`http://localhost:3001/api/teachers/analytics/${cookies.roomId}`,{withCredentials:true});
      ...
      <nav className="bg-white p-12 flex justify-between items-center shadow-sm">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <p className="font-medium text-lg">{cookies.teacherName || 'Teacher'}</p>
