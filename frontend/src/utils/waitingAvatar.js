function positionAvatars() {
	...
	for (let i = 0; i < avatars.length; i++) {
		...
		const _x =
			centerX + radius * Math.cos(i * angleStep) - avatars[i].offsetWidth / 2;
		const _y =
			centerY + radius * Math.sin(i * angleStep) - avatars[i].offsetHeight / 2;

		// Apply positions
		avatars[i].style.position = "absolute";
		avatars[i].style.left = $xpx;
		avatars[i].style.top = $ypx;
	}
}
