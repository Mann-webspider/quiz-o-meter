const { RoomModel, UserModel } = require("../db/redis-models");
	...
	static async checkQuizAnswerAndSubmit(userId, studentAnswers, roomId) {
		try {
			...
			for (const studentAnswer of studentAnswers) {
				...
				switch (quiz.type) {
					case "multiple-choice":
						...
						studentAnswerText = quiz.options[parseInt(studentAnswer.answer, 2)];
					...
					case "multi-select": {
						...
						studentAnswerText = studentAnswersArray
							.map((idx) => quiz.options[parseInt(idx, 2)])
							.join(", ");
			...
			if (user) {
				...
				if (global?.io?.publishUserUpdate) {
